  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --disable-ssh --package-branch tsfc wence/complex --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --no-package-manager || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
